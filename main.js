...
(function () {
  ...
  ════════════════════════════════ */
  function initDoor() {
    const liveDoorScene = document.querySelector('.door-scene');
    if (!liveDoorScene) return;

    // Reset listener by cloning and replacing to prevent duplicates
    const newDoorScene = liveDoorScene.cloneNode(true);
    if (liveDoorScene.parentNode) {
      liveDoorScene.parentNode.replaceChild(newDoorScene, liveDoorScene);
    }

    newDoorScene.addEventListener('click', openDoor, { once: true });
    newDoorScene.addEventListener('touchstart', openDoor, { once: true });
    ...
    const shimmerInterval = setInterval(() => {
      ...
      shimmerTick++;
      const liveDoorLight = document.querySelector('.door-light');
      if (liveDoorLight) {
        liveDoorLight.style.transition = 'opacity 1.5s ease';
        liveDoorLight.style.opacity = shimmerTick % 2 === 0 ? '0.06' : '0.12';
      }
    }, 1500);
  }

  function openDoor() {
    const liveDoorPanelL = document.querySelector('.door-panel.left');
    const liveDoorPanelR = document.querySelector('.door-panel.right');
    const liveDoorLight  = document.querySelector('.door-light');
    const liveDoorBurst  = document.querySelector('.door-burst');
    const liveDoorCta    = document.querySelector('.door-cta');

    if (!liveDoorPanelL || !liveDoorPanelR) return;

    // swing doors open
    liveDoorPanelL.style.transform = 'perspective(1200px) rotateY(80deg)';
    liveDoorPanelR.style.transform = 'perspective(1200px) rotateY(-80deg)';

    // light burst
    if (liveDoorLight) {
      liveDoorLight.style.transition = 'opacity 0.8s ease';
      liveDoorLight.style.opacity = '1';
    }

    // burst orb
    if (liveDoorBurst) {
      liveDoorBurst.style.transition = 'opacity 0.4s ease, transform 1.2s cubic-bezier(.17,.67,.83,.67)';
      liveDoorBurst.style.opacity = '0.95';
      liveDoorBurst.style.transform = 'scale(8)';
    }

    if (liveDoorCta) {
      liveDoorCta.style.transition = 'opacity 0.3s ease';
      liveDoorCta.style.opacity = '0';
    }
  ...
  function revealSite() {
    ...
    const liveSkipBtn = document.getElementById('skip-intro');
    if (liveSkipBtn) {
      liveSkipBtn.classList.remove('show');
    }
    ...
    setTimeout(() => {
      // hide intro
      const liveIntro = document.getElementById('intro');
      if (liveIntro) {
        liveIntro.style.transition = 'opacity 0.6s ease';
        liveIntro.style.opacity = '0';
        setTimeout(() => { liveIntro.style.display = 'none'; }, 650);
      }

      // show main
      const liveMainContent = document.getElementById('main-content');
      if (liveMainContent) {
        liveMainContent.classList.add('visible');
      }
  ...
  function updateDots(index) {
    const liveDots = document.querySelectorAll('.intro-progress .dot');
